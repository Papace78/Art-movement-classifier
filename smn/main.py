...
from smn.no_ai import average_color, dominant_colors, draw_table
...
def main():
    ...
    if WITHOUT_AI:
        change_of_style_index = np.arange(0,len(data_extract), IMAGES_PER_STYLE).astype(int)
        ...
        for i in tqdm(range(len(data_extract))):
            ...
            if i in change_of_style_index:
                ...
                if i == 0:
                    ...
                    np.savetxt(f'{style} dom', np.vstack((dom_col0,
                                                          dom_col1,
                                                          dom_col2,
                                                          dom_col3,
                                                          dom_col4)).astype(int))

                    dominant_palette= np.vstack((canva*dom_col0,
                                                 canva*dom_col1,
                                                 canva*dom_col2,
                                                 canva*dom_col3,
                                                 canva*dom_col4))
            ...
            color0,color1,color2,color3,color4 = dominant_colors(image_path, return_table = False)
    ...
    X_train, X_val, X_test, y_train, y_val, y_test = split_data(data_extract,
                                                                train_size = 0.9,
                                                                val_size = 0.1,
                                                                random_state = 42)
